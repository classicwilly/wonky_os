...
const AllChecklists = () => {
  const groupedBySource = ALL_CHECKLIST_DATA.reduce((acc, section) => {
    ...
    return acc;
    // FIX: Typed the accumulator to help TypeScript's inference.
  }, {} as Record<string, typeof ALL_CHECKLIST_DATA>);
      ...
      >
          ...
          <ul className="list-none">
            {section.items.map((item: any) => (
              <ChecklistItem
                ...
                achievementAwardId={item.id === 'fdp-check-8' ? 'foundational-day-complete' : item.achievementAwardId} // Explicit check for foundational-day-complete
                large={item.large}
              >
                {item.label}
              </ChecklistItem>
        ...
        {section.subSections && (
          <div className="space-y-4">
            {section.subSections.map((subSection: any) => renderSection(subSection, level + 1))}
          </div>
        )}
      </ContentCard>
    );
