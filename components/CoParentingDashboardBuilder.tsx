...
const CoParentingDashboardBuilder = () => {
  ...
  const [tempEnabledModules, setTempEnabledModules] = useState(coParentingDashboardModules);
  ...
  const modulesForTab = ALL_CO_PARENTING_MODULES_CONFIG.filter(m => m.category === activeTab);
    ...
    <div className="max-w-4xl mx-auto py-8">
      <header className="text-center mb-10">
        ...
        <p className="text-lg text-text-light text-opacity-80">
          Select the tools you want active on the Co-Parenting Hub. Core modules cannot be disabled.
        </p>
      </header>
        ...
        <div className="border-b border-gray-700">
            <nav className="-mb-px flex space-x-2 px-4 overflow-x-auto" aria-label="Tabs">
                {Object.entries(moduleCategories).map(([key, label]) => {
                     const modulesInCategory = ALL_CO_PARENTING_MODULES_CONFIG.filter(m => m.category === key);
